...
const FollowersWindow: NextPage<props> = ({ userID }) => {
  ...
  useEffect(() => {
    ...
    const getFollowers = async () =>
      await UserRoute.getSubscribers({ user: userID });
  ...
  const search = async (
    ...
    return await UserRoute.get(
      {
        filter: {
          _id: { $in: followers.map((follower) => follower.subscriber) },
  ...
  useEffect(() => {
    ...
    search(followers, name, abort).then((res) => setAuthors(res?.data));
    return () => abort.abort();
  }, [followers]); // вызов поиска и его результаты
      ...
      <div className={style.title}>
        <div className={style.left}>
          ...
          <TextLoader
            ...
            font={fonts.h14}
            text={
              followers ? (followers.length > 0 ? 'Person' : 'Empty') : null
            }
        ...
        <div style={{ width: '200px' }}>
          <SimpleInput
            ...
            onValidate={(e) => e.target.value.replace(
